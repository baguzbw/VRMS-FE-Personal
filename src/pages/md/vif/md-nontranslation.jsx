...
export default function MDNonTranslation() {
  ...
  useEffect(() => {
    document.title = ' Variable Input Form / Non-Translation - VRMS';
  }, []);
  ...
  const handleSaveTemplate = async () => {
    try {
      const existingTemplatesResponse = await api.get(
        `${import.meta.env.VITE_API_BASE_URL}/master-data/templates/non-translation`
      );
      ...
      if (popupMode === 'add') {
        ...
        const response = await api.patch(
          `${import.meta.env.VITE_API_BASE_URL}/master-data/templates/non-translation/${currentData.template_id}`,
          {
            template_name: currentData.template_name,
            variable: selectedVariables,
          }
  ...
  const handleDeleteTemplate = async () => {
    if (templateToDelete) {
      try {
        await api.delete(
          `${import.meta.env.VITE_API_BASE_URL}/master-data/templates/non-translation/${templateToDelete.template_id}`
        );
                ...
                <div className={dropdownClass}>
                  <ul>
                    <li
                      ...
                      onClick={() => {
                        window.location.href = `${import.meta.env.VITE_API_BASE_URL}/master-data/templates/non-translation/${template.template_id}/export/xlsx`;
                    ...
                    <li
                      className="m-2 py-2 px-3 cursor-pointer hover:bg-[#F4F7FE] flex items-center rounded-[10px]"
                      onClick={() => {
                        window.location.href = `${import.meta.env.VITE_API_BASE_URL}/master-data/templates/non-translation/${template.template_id}/export/csv`;
                        handleHoverDownload(false);
                      }}
                    >
                      <FileCsv size={24} />
    ...
    <div>
      ...
      <div className="px-8 pt-2 pb-8">
        <BaseTable
          ...
          sortOrder={sortOrder}
          apiUrl="master-data/templates/non-translation"
          renderRow={renderRow}
