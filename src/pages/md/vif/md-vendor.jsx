...
export default function MDVendor() {
  ...
  useEffect(() => {
    document.title = ' Variable Input Form / Vendor - VRMS';
  }, []);
  ...
  const handleSaveTemplate = async () => {
    try {
      const existingTemplatesResponse = await api.get(
        `${import.meta.env.VITE_API_BASE_URL}/master-data/templates/vendor`
      );
      ...
      if (popupMode === 'add') {
        ...
        const response = await api.patch(
          `${import.meta.env.VITE_API_BASE_URL}/master-data/templates/vendor/${currentData.template_id}`,
          {
            template_name: currentData.template_name,
            variable: selectedVariables,
          }
  ...
  const handleDeleteTemplate = async () => {
    if (templateToDelete) {
      try {
        await api.delete(
          `${import.meta.env.VITE_API_BASE_URL}/master-data/templates/vendor/${templateToDelete.template_id}`
        );
  ...
  const dropdownClass = `mx-[10px] absolute top-[1px] right-[200px] bg-white border border-gray-200 rounded-[10px] shadow-md w-[131px] h-[110px] ${hoveredDownload ? 'visible opacity-100' : 'invisible opacity-0'} transition-all duration-300`;
                ...
                <div className={dropdownClass}>
                  <ul>
                    <li
                      ...
                      onClick={() => {
                        window.location.href = `${import.meta.env.VITE_API_BASE_URL}/master-data/templates/vendor/${template.template_id}/export/xlsx`;
                    ...
                    <li
                      className="m-2 py-2 px-3 cursor-pointer hover:bg-[#F4F7FE] flex items-center rounded-[10px]"
                      onClick={() => {
                        window.location.href = `${import.meta.env.VITE_API_BASE_URL}/master-data/templates/vendor/${template.template_id}/export/csv`;
                        handleHoverDownload(false);
                      }}
                    >
                      <FileCsv size={24} />
    ...
    <div>
      ...
      <div className="px-8 pt-2 pb-8">
        <BaseTable
          ...
          sortOrder={sortOrder}
          apiUrl="master-data/templates/vendor"
          renderRow={renderRow}
      ...
      >
        <div>
          ...
          <p className="text-[14px] mb-4 mt-2">Variables</p>
