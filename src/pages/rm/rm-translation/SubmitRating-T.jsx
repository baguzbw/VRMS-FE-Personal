...
export default function SubmitRatingTranslation() {
  const { id } = useParams();
  useEffect(() => {
  ...
  const fetchRatingDetails = async (ratingId) => {
    try {
      const response = await api.get(
        `${import.meta.env.VITE_API_BASE_URL}/freelances/translation/${id}/ratings/${ratingId}`
      );
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      const response = await api.post(
        `${import.meta.env.VITE_API_BASE_URL}/freelances/translation/${id}/ratings`,
        formData,
        {
          headers: {
    ...
    <div>
      ...
      <div className="px-8 pt-2 pb-8">
        <BaseTable
          ...
          sortOrder={sortOrder}
          apiUrl={`freelances/translation/${id}/ratings`}
          renderRow={renderRow}
            ...
            <div className="relative mb-4 mt-4">
              <Upload
                fileList={currentData.files.map((file) => ({
                  ...
                  status: 'done',
                  url: `${import.meta.env.VITE_API_BASE_URL}/freelances/translation/${id}/ratings/${currentData.submit_rating_id}/${file.filename}/download`,
                }))}
                showUploadList={{
                  showDownloadIcon: true,
                  downloadIcon: <Download size={16} primaryColor="#0553BA" />,
