...
export default function PMNotesTranslation() {
  const { id } = useParams();
  useEffect(() => {
  ...
  const handleSaveNotes = async () => {
    ...
    if (popupMode === 'add') {
      const response = await api.post(
        `${import.meta.env.VITE_API_BASE_URL}/freelances/translation/${id}/pm-notes`,
        {
      ...
      const response = await api.patch(
        `${import.meta.env.VITE_API_BASE_URL}/freelances/translation/${id}/pm-notes/${currentData.pm_notes_id}`,
        {
          reply: reply,
    ...
    <div>
      ...
      <div className="px-8 pt-2 pb-8">
        <BaseTable
          ...
          sortOrder={sortOrder}
          apiUrl={`freelances/translation/${id}/pm-notes`}
          renderRow={renderRow}
