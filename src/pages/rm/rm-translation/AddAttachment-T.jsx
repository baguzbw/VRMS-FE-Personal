...
export default function RMTranslationAttachment() {
  const { id } = useParams();
  ...
  const handleDelete = async () => {
    if (fileToDelete) {
      try {
        await api.delete(
          `${import.meta.env.VITE_API_BASE_URL}/freelances/translation/${id}/attachments/${fileToDelete.filename}`
        );
        ...
        >
          <div className="mx-[10px] -z-50">
            <button
              ...
              onClick={() => {
                window.location.href = `${import.meta.env.VITE_API_BASE_URL}/freelances/translation/${id}/attachments/${attach.filename}/download`;
              }}
            >
