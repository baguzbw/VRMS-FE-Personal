...
export default function RMTranslationAdd() {
  useEffect(() => {
  ...
  const handleLanguageFromChange = (value) => {
    ...
    console.log('selected language from: ', value);
  };

  const handleLanguageToChange = (value) => {
    setLanguageTo(value);
    console.log('selected language to: ', value);
  };
  ...
  const filterLanguage = (input, languangeoptions) =>
    (languangeoptions?.label ?? '').toLowerCase().includes(input.toLowerCase());
  ...
  const handleSubmitResource = async () => {
    ...
    try {
      const response = await api.post(
        `${import.meta.env.VITE_API_BASE_URL}/freelances/translation`,
        data
      );
      console.log('Data submitted successfully:', response.data);
      console.log('Response data:', response.data);
      const freelanceId = response.data.data.freelance_id;
      navigate(`/resource-manager/translation/${freelanceId}/attachments`);
    } catch (error) {
      console.error(
        'There was an error!',
              ...
              <div className="w-1/2 pl-4">
                ...
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="relative">
                    ...
                    <Select
                      className={`rounded-select mb-6 text-[14px]  ${
                        formSubmitted && isFieldEmpty(languageFrom)
                          ? 'border-[#DC362E] focus:border-[#DC362E]'
                          : 'border-[#BBBBBB] focus:border-[#065BCC]'
                      }`}
                      showSearch
                  ...
                  <div className="relative">
                    ...
                    <Select
                      className={`rounded-select mb-6 text-[14px]  ${
                        formSubmitted && isFieldEmpty(languageTo)
                          ? 'border-[#DC362E] focus:border-[#DC362E]'
                          : 'border-[#BBBBBB] focus:border-[#065BCC]'
                      }`}
                      showSearch
