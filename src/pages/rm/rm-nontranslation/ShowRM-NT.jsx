...
export default function RMNonTranslationShow() {
  const { id } = useParams();
  ...
  useEffect(() => {
    api
      .get(`${import.meta.env.VITE_API_BASE_URL}/master-data/tools`)
      .then((response) => {
  ...
  const handleDeleteFile = async () => {
    if (fileToDelete) {
      try {
        await api.delete(
          `${import.meta.env.VITE_API_BASE_URL}/freelances/non-translation/${data.freelance_id}/attachments/${fileToDelete.filename}`
        );
        ...
        >
          <div className="mx-[10px] -z-50">
            <button
              ...
              onClick={() => {
                window.location.href = `${import.meta.env.VITE_API_BASE_URL}/freelances/non-translation/${data.freelance_id}/attachments/${attach.filename}/download`;
              }}
            >
      ...
      <div className="px-8 pt-10 pb-8">
        <div className="w-full bg-white shadow-md rounded-lg p-8 flex flex-col items-center">
          ...
          <section className="w-full mt-8">
            ...
            <div className="w-full flex justify-between mb-6"></div>
            <div className="w-full">
              <BaseTable
                key={tableKey}
                apiUrl={`freelances/non-translation/${id}/attachments`}
                renderRow={renderRow}
          ...
          <div className="w-full flex justify-end mt-14">
            <Button
              ...
              onClick={() => navigate('/resource-manager/non-translation')}
            >
              <FloppyDisk
