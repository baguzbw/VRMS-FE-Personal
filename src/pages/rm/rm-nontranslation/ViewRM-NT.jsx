...
export default function NonTranslation() {
    ...
    api
      ...
      .then((response) => {
        if (response.data.code === 200) {
          if (action === 'show') {
            navigate(`/resource-manager/non-translation/${id}/show`, {
              state: { resource: response.data.data.freelance },
            });
          } else if (action === 'edit') {
            navigate(`/resource-manager/non-translation/${id}/edit`, {
              state: { resource: response.data.data.freelance },
            });
          }
        } else {
  ...
  const handleDeleteResource = async () => {
    if (resourceToDelete) {
      try {
        await api.delete(
          `${import.meta.env.VITE_API_BASE_URL}/freelances/non-translation/${resourceToDelete.freelance_id}`
        );
  ...
  const handleUpload = async (options) => {
    ...
    try {
      const response = await api.post(
        `${import.meta.env.VITE_API_BASE_URL}/freelances/non-translation/import`,
        formData,
        {
          headers: {
          ...
          >
            <div className="mx-[10px]">
              ...
              <button
                ...
                onClick={() =>
                  navigate(
                    `/resource-manager/non-translation/submit-rating/${resource.freelance_id}`
                  )
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  navigate(
                    `/resource-manager/non-translation/PM-notes/${resource.freelance_id}`
                  )
                }
              >
  ...
  return (
    <div>
      ...
      <div className="p-8 flex justify-between gap-4">
        ...
        <div className="flex gap-4">
              ...
              <div className="absolute top-[60px] right-0 bg-white border border-gray-200 rounded-[10px] shadow-md w-[131px] h-[110px]">
                <ul>
                  <li
                    ...
                    onClick={() =>
                      (window.location.href = `${import.meta.env.VITE_API_BASE_URL}/freelances/non-translation/export/xlsx`)
                    }
                  >
                    <FileXls size={24} />
                  ...
                  <li
                    className="m-2 py-2 px-3 cursor-pointer hover:bg-[#F4F7FE] flex items-center rounded-[10px]"
                    onClick={() =>
                      (window.location.href = `${import.meta.env.VITE_API_BASE_URL}/freelances/non-translation/export/csv`)
                    }
                  >
                    <FileCsv size={24} />
          ...
          <Button
            ...
            onClick={() => navigate('/resource-manager/non-translation/add')}
          >
      ...
      <div className="px-8 pt-2 pb-8 mb-16">
        <BaseTable
          key={tableKey}
          searchQuery={searchQuery}
          sortOrder={sortOrder}
          apiUrl="freelances/non-translation"
          renderRow={renderRow}
