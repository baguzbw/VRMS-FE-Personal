...
export default function RMNonTranslationEdit() {
  const { id } = useParams();
  ...
  const uploadFiles = async () => {
    ...
    try {
      const response = await axios({
        method: 'POST',
        url: `${import.meta.env.VITE_API_BASE_URL}/freelances/non-translation/${id}/attachments`,
        data: formData,
        headers: {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      navigate('/resource-manager/non-translation');
    } catch (error) {
      console.error(
        'There was an error!',
