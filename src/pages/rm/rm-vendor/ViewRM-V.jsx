...
const renderHeader = () => (
  <tr className="bg-[#F9F9F9] text-[#9A9A9A] text-left h-[57px]">
    ...
    <th className="py-3 w-[237px] border-y border-[#E9E9E9]">Vendor Name</th>
...
export default function Vendor() {
    ...
    api
      ...
      .then((response) => {
        if (response.data.code === 200) {
          if (action === 'show') {
            navigate(`/resource-manager/vendor/${id}/show`, {
              state: { resource: response.data.data.vendor },
            });
          } else if (action === 'edit') {
            navigate(`/resource-manager/vendor/${id}/edit`, {
              state: { resource: response.data.data.vendor },
            });
          }
        } else {
  ...
  const handleDeleteResource = async () => {
    if (resourceToDelete) {
      try {
        await api.delete(
          `${import.meta.env.VITE_API_BASE_URL}/vendors/${resourceToDelete.vendor_id}`
        );
  ...
  const handleUpload = async (options) => {
    ...
    try {
      const response = await api.post(
        `${import.meta.env.VITE_API_BASE_URL}/vendors/import`,
        formData,
        {
          headers: {
  ...
  const renderRow = (resource, index, startIndex, showActions) => (
    ...
    >
      ...
      <td className="py-2 border-b border-[#E9E9E9]">
        {resource.vendor_name.length > 15
          ? `${resource.vendor_name.substring(0, 15)}...`
      ...
      {showActions && (
        <td className="py-2 border-b border-r border-[#E9E9E9]">
          <ActionButton
            isOpen={openActionId === resource.vendor_id}
            toggle={() => toggleActionDropdown(resource.vendor_id)}
            label="ACTION"
          >
            <div className="mx-[10px]">
              <button
                className="flex items-center w-full h-[40px] hover:bg-[#F4F7FE] mb-[5px] rounded-[8px]"
                onClick={() => fetchResourceDetails(resource.vendor_id, 'show')}
              >
              ...
              <button
                className="flex items-center w-full h-[40px] hover:bg-[#F4F7FE] mb-[5px] rounded-[8px]"
                onClick={() => fetchResourceDetails(resource.vendor_id, 'edit')}
              >
              ...
              <button
                ...
                onClick={() =>
                  navigate(
                    `/resource-manager/vendor/submit-rating/${resource.vendor_id}`
                  )
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  navigate(
                    `/resource-manager/vendor/PM-notes/${resource.vendor_id}`
                  )
                }
              >
      ...
      <div className="p-8 flex justify-between gap-4">
        ...
        <div className="flex gap-4">
              ...
              <div className="absolute top-[60px] right-0 bg-white border border-gray-200 rounded-[10px] shadow-md w-[131px] h-[110px]">
                <ul>
                  <li
                    ...
                    onClick={() =>
                      (window.location.href = `${import.meta.env.VITE_API_BASE_URL}/vendors/export/xlsx`)
                    }
                  >
                    <FileXls size={24} />
                  ...
                  <li
                    className="m-2 py-2 px-3 cursor-pointer hover:bg-[#F4F7FE] flex items-center rounded-[10px]"
                    onClick={() =>
                      (window.location.href = `${import.meta.env.VITE_API_BASE_URL}/vendors/export/csv`)
                    }
                  >
                    <FileCsv size={24} />
          ...
          <Button
            ...
            onClick={() => navigate('/resource-manager/vendor/add')}
          >
