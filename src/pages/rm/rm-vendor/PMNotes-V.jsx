...
export default function PMNotesVendor() {
  const { id } = useParams();
  useEffect(() => {
  ...
  const handleSaveNotes = async () => {
    ...
    if (popupMode === 'add') {
      const response = await api.post(
        `${import.meta.env.VITE_API_BASE_URL}/vendors/${id}/pm-notes`,
        {
      ...
      const response = await api.patch(
        `${import.meta.env.VITE_API_BASE_URL}/vendors/${id}/pm-notes/${currentData.pm_notes_id}`,
        {
          reply: reply,
